...
import { displayHelpGuideForNuclei } from "@/lib/plugins/plugin-helper/help-guides"
import { transformUserQueryToNucleiCommand } from "@/lib/plugins/plugin-helper/transform-query-to-command"
import { handlePluginStreamError } from "@/lib/plugins/plugin-helper/plugin-stream"
import { processStreamResponse } from "@/lib/tools/tool-helper/tools-stream"
...
export async function handleNucleiRequest(
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      let nucleiUrl = `${process.env.SECRET_GKE_TOOLS_BASE_URL}/api/chat/tools/website-scanner`
      ...
      try {
        const websiteScannerResponse = await fetch(nucleiUrl, {
          method: "POST",
          headers: {
